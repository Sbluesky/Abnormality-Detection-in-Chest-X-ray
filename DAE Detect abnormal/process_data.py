...
class ProcessData():
    ...
    def get_masks(self,img_names,load_dir):
        masks =[]
    ...
    def get_data_noise(self,data,masks,ntype=3,my_seed=None):
        seed = self.seed
        if my_seed is not None:
            seed=my_seed
        #Tạo noise dạng bông tuyết
        snowflake_big = iaa.weather.Snowflakes(density=0.07,density_uniformity=0.5,flake_size=0.98,flake_size_uniformity=0.7,speed=[0.0,0.05],angle=[80,115],seed=seed)
        snowflake_line = iaa.weather.Snowflakes(density=[0.035,0.04],density_uniformity=0.5,flake_size=0.7,flake_size_uniformity=0.7,speed=0.1,angle=[80,115],seed=seed)
        #P1: chọn aplly 1 hoặc 2 cách trên
        someof = iaa.SomeOf((1,2),[snowflake_big,snowflake_line],seed=seed)
        
        #P2: Tạo noise dropout hình
        codrop = iaa.CoarseDropout(p=(0.3,0.4),size_percent=0.009,seed=seed)
